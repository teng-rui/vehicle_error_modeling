...
performance_path = os.path.abspath(os.path.join(path, os.pardir)) + '\\performance\\uncertainty_aware_model\\'
model_path = os.path.abspath(os.path.join(path, os.pardir)) + '\\models\\uncertainty_aware_batch\\'
...
def evaluate(training_inputs, training_targets,val_inputs, val_targets, model):
    ...
    mu = res[:, 0]
    sigma = torch.log(1 + torch.exp(res[:, 1]))
    mse_rnn_val = torch.mean(np.square(val_targets[:, 0] - mu))
    logp_rnn_val = loss_logp_fn(mu, val_targets[:, 0], sigma)
...
def iterate_train(model, optimizer, training_inputs, training_targets, val_inputs, val_targets, batchsize,
    ...
    performance = pd.DataFrame(index=range(epochs), columns=['mse_training','mse_val','logp_training', 'logp_val'])
    for epoch in range(epochs):
        print('epoch', epoch)
        train(training_input_batch, training_target_batch, sequenceLength, model, optimizer)

        mse_rnn_training, mse_rnn_val,logp_rnn_training,logp_rnn_val = evaluate( training_inputs, training_targets, val_inputs, val_targets, model=model)
        print(mse_rnn_training, mse_rnn_val,logp_rnn_training,logp_rnn_val)
        performance.iloc[epoch, :] = [mse_rnn_training.numpy(), mse_rnn_val.numpy(),logp_rnn_training.numpy(),logp_rnn_val.numpy()]
    performance.to_csv(performance_path + experiment_name + '.csv')
    torch.save(model.state_dict(), '../models/uncertainty_aware_batch/epoch_' + str(epochs) + experiment_name + '.pt')
...
if __name__ == '__main__':
    ...
    for item in list(itertools.product(args.batchsize_list, args.tbptt_list, args.lr_list)):
        ...
        model = NeuralNetwork(internal_layers=args.layer, hidden_dim=args.node, input_dim=18, output_dim=2,
