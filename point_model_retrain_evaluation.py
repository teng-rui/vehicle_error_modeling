...
def evaluate(training_inputs, training_targets,testing_inputs, testing_targets, model):
    ...
    res_mu = model(torch.nn.utils.rnn.pack_sequence(inputs_).to(device), initial_hidden=True).data.detach().cpu()
    mse_rnn_testing = torch.mean(np.square(testing_targets - res_mu))
    return mse_rnn_training, mse_rnn_testing
...
def iterate_train(model, optimizer, training_inputs, training_targets, testing_inputs, testing_targets, batchsize,
    ...
    mse_training, mse_testing = evaluate(training_inputs, training_targets,testing_inputs, testing_targets, model=model)
    print(mse_training, mse_testing)
    torch.save(model.state_dict(), '../models/point_prediction_batch/retrain_epoch_' + str(epochs) + experiment_name + '.pt')
...
if __name__ == '__main__':
    ...
    # initialize models
    model = NeuralNetwork(internal_layers=args.layers, hidden_dim=args.nodes, input_dim=18, output_dim=1,
